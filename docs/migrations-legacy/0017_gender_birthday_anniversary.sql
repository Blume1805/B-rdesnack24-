-- ============================================================================
-- 0017 — Geschlecht, angepasste Loyalty-Rabatte, Geburtstags- und Jahresangebote
-- ----------------------------------------------------------------------------
-- * profiles.gender ('m' | 'w' | 'd') als optionales Stammdatum.
-- * Loyalty-Rabatte auf 5/10/15/25 % pro Meilenstein angepasst.
-- * Geburtstagsangebot: 50 % Rabatt, 14 Tage gültig, source='birthday'.
-- * Jahresangebot (jährlich am Registrierungs-Jubiläum): 30 % Rabatt,
--   14 Tage gültig, source='anniversary'.  Kunde wählt das Produkt selbst
--   (Client übergibt product_id an public.redeem_special_offer_choose_product,
--   Backend legt entsprechend das Angebot fest — hier bewusst schmal
--   gehalten: wir generieren einen "Wildcard"-Rabattcode; das eigentliche
--   Rabatt-Produkt wird beim Einlösen an den Automaten übermittelt).
-- * unique-Index nur noch für Basis-Angebote (source='auto'), damit
--   Loyalty-/Geburtstags-/Jahresangebote parallel existieren dürfen.
-- ============================================================================
...
-- 3) Unique-Regel: nur EIN Basis-Angebot gleichzeitig ------------------------
-- Loyalty-/Birthday-/Anniversary-Angebote dürfen parallel bestehen.
...
-- 4) Wildcard-Produkt für Geburtstags-/Jahresangebote ------------------------
-- Der Kunde wählt das Produkt beim Einlösen selbst.  Wir speichern deshalb
-- ein Platzhalter-Produkt und Preise 0 — der Automatentransfer setzt den
-- konkreten Preis beim Einlösevorgang.  Damit die App den Preisunterschied
-- trotzdem anzeigen kann, führen wir title = 'Produkt deiner Wahl' und den
-- Rabattprozentsatz mit; Frontend zeigt "auf jedes Produkt".
...
begin
  ...
  -- Nur am Tag & Monat des Geburtstags:
  ...
  -- Bereits eines dieses Jahr vergeben?
  if exists (
...
-- 6) Jahresangebot (Registrierungs-Jubiläum) ---------------------------------
...
-- 7) Täglicher Sammel-Job — kann per pg_cron 08:00 Europe/Berlin laufen -----
...
-- 8) Client-seitige Auto-Trigger — beim Öffnen der App checken -------------
-- Diese Funktion ist idempotent: Sie erzeugt Geburtstags-/Jahresangebot für
-- den aktuellen Nutzer NUR wenn heute der passende Tag ist und noch keins
-- vergeben wurde.
