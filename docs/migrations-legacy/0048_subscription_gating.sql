-- 0048 — Abo-Gating (Geschäftsentscheidung: Basis frei, Vorteile im Abo).
--
-- Ohne Abo bleiben frei: Automatenfinder, Verfügbarkeit, Kaufhistorie,
-- Profil, News, Community-Favoriten. Abo-Vorteile: Coupons/Angebote
-- aktivieren, Deals, Loyalty-Punkte inkl. Meilenstein-Boni,
-- personalisierte Angebote.
--
-- Durchsetzung serverseitig (die UI blendet zusätzlich aus):
--   * activate_offer / activate_personal_offer → Fehler ohne Abo.
--   * grant_loyalty_bonuses → vergibt ohne Abo keine Boni (return 0),
--     der Kauf-Trigger läuft dadurch für Nicht-Abonnenten ins Leere.
--
-- "Abo aktiv" heißt bis zur Store-Zahlung: Kunde hat ein Modell gewählt
-- (customer_subscriptions hat mindestens eine Zeile — Lifetime sowieso,
-- Monats-/Jahres-Abo gelten als aktiv, solange keine Kündigung
-- verarbeitet wurde; die Kündigungsabwicklung übernimmt später der Store).
...
-- ── Loyalty-Boni nur für Abonnenten ─────────────────────────────────────
-- Volle Neudefinition (Stand 0037) + Gating-Frühausstieg.
...
begin
  -- Abo-Gating: Loyalty ist Abo-Vorteil.
