-- ============================================================================
-- 0063 · Finanz-Etappe 2 — Bilanz-Sync-Schreibpfad (sevDesk / Service-Role)
-- ----------------------------------------------------------------------------
-- Der sevDesk-Bilanz-Sync (Edge Function finance-balance-sync) läuft mit der
-- Service-Role und schreibt Snapshots mit source='sevdesk'. Der bestehende
-- upsert_finance_balance erzwingt source='manual' + Rollencheck auf auth.uid()
-- und ist damit für den Sync nicht nutzbar. Diese Variante ist AUSSCHLIESSLICH
-- der Service-Role zugänglich (kein Grant an authenticated) und setzt
-- source='sevdesk'.
-- ============================================================================
...
-- Bewusst NUR Service-Role (der Sync läuft mit Service-Key). Kein Grant an
-- authenticated — Kund:innen/Mitarbeiter können source='sevdesk' nicht setzen.
