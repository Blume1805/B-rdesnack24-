...
# Berechnung
# ===========================================================================
def berechne(umsatz_faktor=1.0):
    """Rechnet das Modell einmal komplett durch. umsatz_faktor skaliert nur
    REVENUE_BRUTTO_PRO_MONAT (Bruttoumsatz je Automat und Monat) -- das ist
    die unsicherste Annahme im Modell (siehe Abschnitt 4/6 des Businessplans),
    alle uebrigen Annahmen bleiben unveraendert. 1.0 = Planungsszenario,
    0.6 = pessimistisch (-40 %), 1.4 = optimistisch (+40 %)."""
    kumuliert = []
    ergebnisse = []
    gesamt_automaten = 0

    for y in YEARS:
        neue = [dict(m) for m in MACHINES.get(y, [])]
        for m in neue:
            m["jahr_start"] = y
