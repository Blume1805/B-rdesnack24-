...
| A-12 | **Die Auslieferung ueber GitHub Actions kann keine funktionsfaehige App bauen: Die noetigen Zugangsdaten fehlen als Repository-Secrets.** `.github/workflows/deploy-web.yaml` laeuft bei jedem Push auf `main` und ruft `scripts/deploy_web.sh`. Das Skript baut mit `--dart-define-from-file=apps/mobile/env/demo.json`; dieser Pfad steht in `.gitignore` und war **nie** im Repository (gesucht ueber alle Zweige). Ohne `SUPABASE_URL` und `SUPABASE_ANON_KEY` ist `AppConfig.isValid` false und `bootstrap.dart` zeigt statt der App einen Konfigurationshinweis. **Nachgewiesen am 2026-09-17** im Lauf 32: Beide Secrets sind leer — das Protokoll zeigt `SUPABASE_URL:` und `SUPABASE_ANON_KEY:` ohne Wert. Der zuletzt ausgelieferte Stand (gh-pages, 04.09.2026) traegt die Projekt-URL im Bundle und wurde deshalb **von Hand** erzeugt, nicht ueber diesen Workflow. Zweiter, davon unabhaengiger Punkt: Ohne `CUSTOM_DOMAIN` kompiliert das Skript den Basispfad `/B-rdesnack24-/` ein und schreibt keine `CNAME`-Datei — unter der bereits aufgeloesten Adresse `app.boerdesnack24.de` waere eine weisse Seite erschienen. | **teilweise behoben am 2026-09-17.** Der Workflow schreibt die Konfiguration jetzt aus den Repository-Secrets, setzt `CUSTOM_DOMAIN=app.boerdesnack24.de` und **bricht laut ab**, wenn eines der beiden Geheimnisse fehlt, statt eine App ohne Datenbankverbindung zu veroeffentlichen. Genau das ist im Lauf 32 passiert: Die Auslieferung stoppte vor dem Bauen, es wurde nichts veroeffentlicht, der Live-Stand blieb unveraendert. 🔴 **offen:** Die beiden Secrets muessen gesetzt werden — `docs/OPERATIONS.md`, Runbook J. Verantwortlich: Philipp Blume. **Nicht geklaert:** Lauf 31 (27.08.2026, manuell ausgeloest) meldet Erfolg, obwohl Skript und fehlende Datei damals dieselben waren. Warum, laesst sich nachtraeglich nicht feststellen; der Punkt wird hier nur festgehalten, nicht erklaert. **Zwischenstand 17.09.2026, 12:30 Uhr:** Die Secrets sind gesetzt, aber `SUPABASE_URL` enthaelt keinen Adress-, sondern einen Schluesselwert (208 Zeichen; eine Projektadresse hat rund 40). Lauf 36 hat deshalb ein Paket veroeffentlicht, dessen Datenbankadresse unbrauchbar ist — die App unter `app.boerdesnack24.de` laedt, kann sich aber nicht verbinden. Der Zustand ist schlechter als der Stand vom 04.09., der funktionierte. **Kein Geheimnis ist ausgetreten:** Der im Paket verbaute Schluessel wurde dekodiert und traegt `role: anon`, Projekt `nnfsyuglkqycwenwxmuw`, gueltig bis 2036 — ein Wert, der bestimmungsgemaess oeffentlich ist. Ein `service_role`-Schluessel ist nicht enthalten (im Paket steckt genau ein Token). Lauf 37 prueft seit demselben Tag die Form beider Werte und hat den Fehler benannt, statt erneut zu veroeffentlichen. **Abgeschlossen am 17.09.2026, 12:38 Uhr (Lauf 40).** Der korrigierte Wert trug zunaechst noch einen Zeilenumbruch, den die damalige `grep`-Pruefung durchliess (grep vergleicht zeilenweise, die erste Zeile passte); er brach die Uebersetzung ab. Die Pruefung saeubert die Werte jetzt erst (Umbrueche, Tabulatoren, Leerzeichen, abschliessender Schraegstrich) und vergleicht mit `[[ =~ ]]` gegen die vollstaendige Zeichenkette. Acht Faelle erprobt, darunter beide echten Fehlschlaege. **Am ausgelieferten Paket nachgeprueft:** `CNAME` = `app.boerdesnack24.de`, Basispfad `/`, Projektadresse `https://nnfsyuglkqycwenwxmuw.supabase.co` enthalten, Schluessel dekodiert (`role: anon`, richtiges Projekt), Rechtstexte in der Fassung `v3 · 2026-09`, keine Fundstelle mehr fuer `9,99 €` oder `Plus-Abo`. **Nicht selbst geprueft:** dass die Seite im Browser tatsaechlich laedt — der Netzwerkfilter dieser Arbeitsumgebung erreicht die Adresse nicht. **Lehre:** Ein Auslieferungsweg, dessen Voraussetzungen nie geprueft wurden, ist eine Vermutung — und ein gruener Haken von vor drei Wochen belegt nicht, dass er heute traegt. |
