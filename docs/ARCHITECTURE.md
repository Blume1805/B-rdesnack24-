...
| A-12 | **Der Auslieferungs-Workflow haette die funktionierende App durch eine tote ersetzt.** `.github/workflows/deploy-web.yaml` laeuft bei jedem Push auf `main` und ruft `scripts/deploy_web.sh` ohne Umgebungsdatei und ohne `CUSTOM_DOMAIN` auf. `apps/mobile/env/*.json` steht in `.gitignore` — in der Werkbank existiert die Datei also nicht. Das Skript haette ohne `SUPABASE_URL` und `SUPABASE_ANON_KEY` gebaut; `AppConfig.isValid` waere false und die App haette dem Nutzer nur noch `nicht konfiguriert` gezeigt. Unbemerkt geblieben ist das, weil der letzte Deploy (04.09.2026, gh-pages) **von Hand** lief, mit einer lokal vorhandenen `demo.json` — im ausgelieferten Bundle steht die Projekt-URL nachweislich drin. Der erste Lauf des Workflows waere der erste ohne sie gewesen. Zweiter Punkt: Ohne `CUSTOM_DOMAIN` kompiliert das Skript den Basispfad `/B-rdesnack24-/` ein und schreibt keine `CNAME`-Datei — unter der bereits aufgeloesten Adresse `app.boerdesnack24.de` waere eine weisse Seite erschienen. | **behoben am 2026-09-17**, bevor der Workflow zum ersten Mal lief. Der Workflow schreibt die Umgebungsdatei jetzt aus den Repository-Secrets und **bricht laut ab**, wenn eines der beiden Geheimnisse fehlt, statt eine App ohne Datenbankverbindung zu veroeffentlichen. `CUSTOM_DOMAIN` ist auf `app.boerdesnack24.de` gesetzt. **Lehre:** Ein Auslieferungsweg, der nie gelaufen ist, ist kein Auslieferungsweg, sondern eine Vermutung. |
