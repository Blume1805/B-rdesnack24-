...
// beim Anbieter verifiziert ist und die MX-Einträge dorthin zeigen.
// Solange das nicht eingerichtet ist, ruft niemand diese Function auf —
// sie schadet aber auch nicht, weil sie ohne Secret gar nicht arbeitet.
//
// KEIN JWT, DESHALB SIGNATUR. Ein Webhook kann sich nicht anmelden; er
// weist sich über eine HMAC-Signatur aus. Diese Function läuft deshalb mit
// `verify_jwt = false` — und trägt ihre Zugangsprüfung selbst. Genau das
// ist die Bauart, die bei `install-signature` schiefgegangen ist, deshalb
...
//   3. Der Zeitstempel wird mitgeprüft, damit eine einmal mitgeschnittene
//      Anfrage sich nicht beliebig oft wiedereinspielen lässt.
//
...
Deno.serve(async (req) => {
  ...
  if (error) {
    ...
    // Eingang erneut zugestellt (übliches Verhalten nach
    // Zeitüberschreitung). Das ist kein Fehler: mit 200 antworten, damit
    // er aufhört zu wiederholen.
    if (error.code === "23505") {
