...
--   period.{from,to,days,prior_year,prior_period}
...
--   customer                               -- App-Käufe, aktive Kunden, Basket
--   derived.{gross_margin_pct, net_margin_pct, ebitda_margin_pct,
--            cashflow_operating,
--            revenue_growth_yoy_pct, revenue_growth_mom_pct,
--            result_growth_yoy_pct,  result_growth_mom_pct}
--
-- Etappe 2 hängt Bilanz-Kennzahlen (Liquidität/EK/ROI) an derived an,
-- sobald finance_balance_snapshots gefüllt sind.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'label',            coalesce(m_name, m_location, 'unbekannt'),
    'code',             m_code,
  ...
  from (
    select
      pu.machine_id,
      (select name          from public.machines where id = pu.machine_id) as m_name,
      (select location_name from public.machines where id = pu.machine_id) as m_location,
      (select code          from public.machines where id = pu.machine_id) as m_code,
