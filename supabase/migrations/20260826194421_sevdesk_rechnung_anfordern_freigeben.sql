...
begin
  if not (public.is_admin() or public.auth_has_permission('businesses.manage')) then
    raise exception 'Keine Berechtigung fuer Firmenkunden' using errcode = '42501';
  end if;
  ...
  -- § 14 UStG verlangt den vollstaendigen Namen und die Anschrift des
  -- Empfaengers. Fehlt das, entsteht keine ordnungsgemaesse Rechnung — dann
  ...
  if coalesce(btrim(b.billing_street), '') = ''
    ...
    raise exception
      'Rechnungsanschrift des Firmenkunden ist unvollstaendig (Strasse, PLZ, Ort).'
      using errcode = '22023';
  end if;
  if coalesce(btrim(b.sevdesk_contact_id), '') = '' then
    raise exception
      'Fuer diesen Firmenkunden ist kein sevDesk-Kontakt hinterlegt.'
      using errcode = '22023';
  end if;
  ...
  if (basis->>'kaeufe')::int = 0 then
    raise exception 'In diesem Monat gibt es keine Firmenkaeufe.'
      using errcode = 'P0002';
  end if;
  ...
  if jsonb_array_length(basis->'probleme') > 0 then
    ...
    raise exception 'Rechnung nicht moeglich: %', probleme using errcode = '22023';
  end if;
...
exception
  when unique_violation then
    raise exception
      'Fuer diesen Monat wurde bereits eine Rechnung angefordert.'
      using errcode = '23505';
end;
$function$;
...
-- wenn die Rechnung in sevDesk tatsaechlich entstanden ist. Eine Freigabe
-- fuer etwas, das es dort noch nicht gibt, waere eine Freigabe ins Leere.
...
begin
  ...
  if lauf.status <> 'in_sevdesk' then
    raise exception
      'Diese Rechnung liegt noch nicht in sevDesk. Erst uebertragen, dann freigeben.'
      using errcode = '22023';
  end if;
