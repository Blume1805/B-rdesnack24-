...
create table if not exists public.document_versions (
  ...
  version integer not null, file_path text not null, notes text,
  created_at timestamptz not null default now(), created_by uuid,
...
set search_path = public, app
as $$
declare next_v int; rec public.document_versions;
begin
...
create table if not exists public.ifsg_briefings (
  ...
  first_briefing date not null, next_due date,
  certificate_path text, created_at timestamptz not null default now(), created_by uuid
);
...
create table if not exists public.employee_trainings (
  ...
  signature_path text, created_at timestamptz not null default now(), created_by uuid
);
