-- ============================================================================
-- 0013 — Datenschutz-Tooling: Auskunft, Löschung, Consent-Erweiterung
-- ----------------------------------------------------------------------------
-- Setzt Betroffenenrechte nach DSGVO Art. 15/17/20 technisch um:
--   * export_my_data() → JSON-Auszug der eigenen Datensätze (Datenauskunft +
--     Übertragbarkeit).
--   * account_deletion_requests + request_account_deletion() → Löschantrag,
--     der von einem Administrator geprüft wird (kein sofortiges Hard-Delete —
--     GoBD-Aufbewahrungsfristen kollidieren; Anonymisierung/Sperre statt Löschung
--     für alte Daten).
--   * app.consent_type erweitert um 'terms' (AGB-Zustimmung).
-- ============================================================================

-- Consent-Typ „terms" (AGB-Zustimmung) ergänzen ------------------------------
do $$ begin
  alter type app.consent_type add value if not exists 'terms';
exception when others then null; end $$;

-- Antrags-Tabelle für Kontolöschung ------------------------------------------
...
comment on function public.export_my_data() is
  'Liefert alle personenbezogenen Daten des Aufrufers als JSON (DSGVO Art. 15/20).';
comment on function public.request_account_deletion(text) is
  'Stellt einen Antrag auf Kontolöschung (DSGVO Art. 17). Wegen GoBD wird kein Hard-Delete durchgeführt.';
