-- ============================================================================
-- 0006 — Unternehmensverwaltung: Stammdaten & Echtzeit-Inventur
-- ----------------------------------------------------------------------------
-- machines, products, inventory (+Trigger), inventory_movements (append-only),
-- machine_stock (View), nayax_sales (Roh-Ingest). RLS + Audit.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- machines (Automaten / Standortliste, Anlage C der Verfahrensdokumentation)
-- ----------------------------------------------------------------------------
create table if not exists public.machines (
  id            uuid primary key default gen_random_uuid(),
  code          text not null unique,            -- interne Automaten-Nr.
  name          text not null,
  ...
  nayax_terminal_id text unique,                 -- Verknüpfung zur Telemetrie
...
create table if not exists public.products (
  ...
  category    text,
  ean         text,
  tax_rate    numeric(4,1) not null default 19.0,  -- 7 | 19
  status      app.entity_status not null default 'active',
  created_at  timestamptz not null default now(),
  created_by  uuid,
  updated_at  timestamptz not null default now(),
...
-- ----------------------------------------------------------------------------
-- inventory (aktueller Bestand je Automat+Produkt)
-- ----------------------------------------------------------------------------
create table if not exists public.inventory (
  ...
  capacity    integer,
  par_level   integer not null default 2,        -- Schwelle "bald leer"
  updated_at  timestamptz not null default now(),
  updated_by  uuid,
...
-- ----------------------------------------------------------------------------
-- inventory_movements (append-only Bewegungen; Quelle der Bestandsänderung)
-- ----------------------------------------------------------------------------
create table if not exists public.inventory_movements (
  ...
  quantity_delta integer not null,               -- +Auffüllen / -Verkauf/Verderb
  reason       text,
  source       app.movement_source not null default 'manual',
  source_ref   text,
...
-- Bestand aus Bewegung fortschreiben (atomar, idempotent je Bewegung) --------
...
-- ----------------------------------------------------------------------------
-- nayax_sales (Roh-Ingest der Telemetrie, dedupliziert)
-- ----------------------------------------------------------------------------
create table if not exists public.nayax_sales (
  ...
  source_ref   text unique,                       -- Nayax-Transaktions-ID (Dedupe)
  raw          jsonb,
  created_at   timestamptz not null default now()
);
...
-- ----------------------------------------------------------------------------
-- machine_stock (View: Verfügbarkeit je Automat+Produkt)
-- ----------------------------------------------------------------------------
...
-- Stammdaten lesbar für alle Angemeldeten (Standorte sind öffentlich; Kunden
-- benötigen Verfügbarkeit). Schreiben: inventory.edit / Admin.
...
-- Bestand lesbar für alle Angemeldeten (Verfügbarkeit). Schreiben serverseitig
-- bzw. via Bewegungen; direktes Schreiben nur inventory.edit/Admin.
...
-- Bewegungen: lesen mit inventory.view; einfügen mit inventory.edit. Kein
...
-- Nayax-Rohdaten: nur Service-Role (Edge Function) schreibt; lesen inventory.view.
