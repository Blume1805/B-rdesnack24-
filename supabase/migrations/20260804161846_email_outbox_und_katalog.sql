...
comment on column public.email_templates.category is
  'transactional = Vertragspost, geht immer raus. marketing = Werbung, braucht Einwilligung nach § 7 Abs. 2 Nr. 2 UWG. legal = Pflichtinformation (AGB-/Datenschutzänderung, Sicherheitshinweis), geht ebenfalls immer raus.';
comment on column public.email_templates.precondition is
  'Wenn gesetzt: was fehlt, bevor diese Vorlage überhaupt ausgelöst werden kann. Gehört mit is_active = false zusammen.';

insert into public.email_templates
  (key, label, description, category, topic_key, variables, legal_note, is_active, precondition)
values
  ('konto_willkommen', 'Willkommen nach bestätigter Registrierung',
   ...
   'Vertragspost. Kein Abmeldelink, keine Werbung im Text — sonst wird aus der Begrüssung eine Werbemail, die eine Einwilligung bräuchte.',
   true, null),
  ('konto_passwort_geaendert', 'Passwort wurde geändert',
   'Sicherheitsmeldung nach erfolgreicher Passwortänderung.',
   'transactional', null, array['vorname','zeitpunkt','support_mail'],
   'Muss auch dann raus, wenn die Änderung erwünscht war — der Zweck ist, dass eine UNerwünschte Änderung auffällt.',
   true, null),
  ('konto_email_geaendert', 'E-Mail-Adresse wurde geändert (Meldung an die alte Adresse)',
   ...
   'Bewusst an die alte Adresse. Wer ein Konto übernimmt, ändert zuerst die E-Mail — eine Meldung nur an die neue Adresse würde niemanden warnen.',
   true, null),
  ('konto_neue_anmeldung', 'Anmeldung von einem unbekannten Gerät',
   ...
   'Speichert Gerätemerkmale und damit personenbezogene Daten. Braucht vor Inbetriebnahme eine Löschfrist im Verzeichnis der Verarbeitungstätigkeiten.',
   false, 'Es gibt kein Geräte-/Sitzungsverzeichnis. Ohne das ist „unbekanntes Gerät" nicht feststellbar.'),
  ('konto_zwei_faktor', 'Zwei-Faktor-Anmeldung: Bestätigungscode',
   'Code für die Anmeldung in zwei Schritten.',
   'transactional', null, array['vorname','code','gueltig_minuten'], null,
   false, 'Zwei-Faktor-Anmeldung ist nicht eingeführt.'),
  ('abo_kuendigung_laufzeitende', 'Kündigung bestätigt, mit Laufzeitende',
   'Nennt ausdrücklich den Tag, bis zu dem das Abo noch läuft.',
   'transactional', null, array['vorname','abo_name','laufzeit_ende','app_link'],
   'Nach § 312k Abs. 2 BGB muss die Kündigung in Textform bestätigt werden, einschliesslich des Zeitpunkts, zu dem sie wirkt. Das Datum ist deshalb kein Beiwerk, sondern der Kern dieser Mail.',
   true, null),
  ('abo_laeuft_bald_ab', 'Erinnerung kurz vor Ablauf',
   'Hinweis, dass das Abo demnächst endet.',
   'transactional', null, array['vorname','abo_name','laufzeit_ende','app_link'],
   'Grenzfall: Solange sie nur über das Ende informiert, ist sie Vertragspost. Sobald sie zur Verlängerung überredet, wird sie Werbung und braucht eine Einwilligung. Beim Texten darauf achten.',
   true, null),
  ('abo_rechnung', 'Rechnung', 'Rechnung zum Abo.',
   'transactional', null, array['vorname','rechnungsnummer','betrag','zeitraum','pdf_link'],
   'Rechnungsangaben nach § 14 UStG sind Pflicht: Steuernummer bzw. USt-IdNr., fortlaufende Nummer, Leistungszeitraum, Nettobetrag, Steuersatz und -betrag. Eine unvollständige Rechnung kostet den Kunden den Vorsteuerabzug.',
   false, 'Kein Zahlungsanbieter angebunden. Ohne Zahlungsdaten gibt es keinen Rechnungsbetrag.'),
  ...
  ('abo_zahlung_fehlgeschlagen', 'Zahlung fehlgeschlagen', 'Hinweis, dass eine Zahlung nicht durchging.',
   ...
   'Muss sagen, was passiert, wenn nichts geschieht — sonst ist die Folge (Sperrung) für den Kunden überraschend.',
   false, 'Kein Zahlungsanbieter angebunden.'),
  ...
  ('abo_verlaengert', 'Abo automatisch verlängert', 'Meldung über die automatische Verlängerung.',
   ...
   'Bei automatischer Verlängerung ist rechtzeitig vorher zu informieren. Diese Mail ist die Umsetzung davon — sie darf nicht erst nach der Abbuchung kommen.',
   false, 'Kein Zahlungsanbieter angebunden.'),
  ...
  ('abo_test_endet_bald', 'Erinnerung vor Ende der Testphase', 'Vorwarnung vor dem Übergang in ein bezahltes Abo.',
   ...
   'Die wichtigste Mail der ganzen Reihe. Wer hier nicht deutlich vorwarnt, produziert Rücklastschriften und berechtigte Beschwerden.',
   false, 'Es gibt kein Testphasen-Modell in den Abodaten.'),
  ('abo_test_beendet', 'Testphase beendet, Abo aktiv', 'Übergang in das bezahlte Abo.',
   ...
   false, 'Es gibt kein Testphasen-Modell in den Abodaten.'),
  ('premium_willkommen', 'Willkommen bei Premium',
   'Begrüssung nach Abschluss eines bezahlten Abos.',
   'transactional', null, array['vorname','abo_name','app_link'],
   'Vertragspost, solange sie erklärt, was jetzt zum Abo gehört. Wird sie zur Anpreisung weiterer Angebote, ist sie Werbung.',
   true, null),
  ('news_neue_funktionen', 'Neue Funktionen in der App',
   'Was seit dem letzten Mal dazugekommen ist.',
   'marketing', 'produkt_neuigkeiten', array['vorname','app_link','abmelde_link'],
   'Werbung. Ohne Einwilligung unzulässig (§ 7 Abs. 2 Nr. 2 UWG). Abmeldelink und Impressum sind Pflicht.',
   true, null),
  ('news_app_update', 'App-Update verfügbar', 'Hinweis auf eine neue Version.',
   'marketing', 'produkt_neuigkeiten', array['vorname','version','app_link','abmelde_link'],
   'Werbung — auch wenn es technisch klingt. Ausnahme wäre nur ein Update, das eine Sicherheitslücke schliesst; das gehört dann unter recht_sicherheitshinweis und nicht hierher.',
   true, null),
  ('aktion_saisonal', 'Saisonale Aktion', 'Aktion oder Rabatt, zeitlich begrenzt.',
   'marketing', 'aktionen', array['vorname','aktion_titel','aktion_text','gueltig_bis','app_link','abmelde_link'],
   'Werbung. Befristete Angebote müssen die Frist wahrheitsgemäss nennen — ein „nur heute", das jede Woche wiederkommt, ist irreführend (§ 5 UWG).',
   true, null),
  ('geburtstag_gutschein', 'Geburtstagsgutschein', 'Einmal im Jahr ein Gutschein zum Geburtstag.',
   'marketing', 'geburtstag', array['vorname','gutschein_code','gueltig_bis','app_link','abmelde_link'],
   'Werbung, und zusätzlich heikel: Sie verarbeitet das Geburtsdatum zu einem Zweck, für den es nicht erhoben wurde. Die Einwilligung für das Thema „geburtstag" deckt genau das ab — deshalb ein eigenes Thema und nicht mit „aktionen" zusammengelegt.',
   true, null),
  ('empfehlung_freund_eingeloest', 'Deine Empfehlung wurde eingelöst',
   ...
   'Vertragspost innerhalb des Empfehlungsprogramms: Sie berichtet über einen erworbenen Anspruch, sie wirbt nicht. Deshalb ohne Einwilligung zulässig — sie darf dann aber auch nichts anderes bewerben.',
   true, null),
  ('support_eingegangen', 'Anfrage ist eingegangen',
   'Eingangsbestätigung für eine Reklamation oder Anfrage.',
   'transactional', null, array['vorname','vorgang_nummer','betreff','eingegangen_am'], null, true, null),
  ('support_beantwortet', 'Anfrage wurde beantwortet',
   'Meldung, dass es eine Antwort zu einem Vorgang gibt.',
   'transactional', null, array['vorname','vorgang_nummer','betreff','antwort_text','app_link'], null, true, null),
  ('support_feedback_erhalten', 'Danke für die Rückmeldung',
   'Bestätigung für eingegangenes Feedback.',
   'transactional', null, array['vorname'],
   'Nur Bestätigung. Sobald daran eine Bitte um eine Bewertung hängt, ist es Werbung.',
   false, 'Es gibt keinen Feedback-Eingang ausserhalb der Reklamationen.'),
  ('recht_datenschutz_aenderung', 'Änderung der Datenschutzhinweise',
   'Information über geänderte Datenschutzhinweise.',
   'legal', null, array['vorname','wirksam_ab','zusammenfassung','link'],
   'Pflichtinformation, keine Werbung — geht deshalb an alle, unabhängig von jeder Einwilligung, und trägt bewusst KEINEN Abmeldelink.',
   true, null),
  ('recht_agb_aenderung', 'Änderung der AGB',
   'Information über geänderte Geschäftsbedingungen.',
   'legal', null, array['vorname','wirksam_ab','zusammenfassung','link','widerspruch_bis'],
   'Muss die Änderung, den Zeitpunkt des Wirksamwerdens und das Widerspruchsrecht nennen. Schweigen darf nur dann als Zustimmung gelten, wenn genau darauf hingewiesen wurde.',
   true, null),
  ('recht_sicherheitshinweis', 'Wichtiger Sicherheitshinweis',
   ...
   'Bei einer Verletzung des Schutzes personenbezogener Daten mit hohem Risiko ist die betroffene Person unverzüglich zu benachrichtigen (Art. 34 DSGVO). Diese Vorlage ist der Weg dafür — sie darf nie deaktiviert werden.',
   true, null)
...
create index if not exists email_outbox_profil_idx on public.email_outbox (profile_id);
create index if not exists email_outbox_log_idx on public.email_outbox (email_log_id);
create index if not exists email_outbox_template_idx on public.email_outbox (template_key);
...
comment on table public.email_outbox is
  'Versandwarteschlange. Wer eine Mail auslösen will, ruft email_enqueue auf und legt hier eine Zeile ab — er verschickt nicht selbst.';
...
comment on function public.email_enqueue(text,uuid,jsonb,text,timestamptz,text) is
  'Einziger Weg in die Warteschlange. Setzt die Einwilligungssperre für Werbung durch und lässt Vertragspost immer durch.';

create or replace function public.email_outbox_claim(p_limit int default 25)
returns table (
  id uuid, template_key text, profile_id uuid, to_address text,
  vars jsonb, category text, attempts int
)
language plpgsql
security definer
set search_path to 'public', 'app'
...
begin
  ...
  returning o.id, o.template_key, o.profile_id, o.to_address, o.vars,
            (select t.category from public.email_templates t where t.key = o.template_key),
            o.attempts;
end;
$function$;

create or replace function public.email_outbox_mark(
  p_id uuid, p_status text, p_error text default null, p_log_id uuid default null
)
returns boolean
language plpgsql
security definer
...
revoke all on function public.email_enqueue(text,uuid,jsonb,text,timestamptz,text) from public, anon, authenticated;
revoke all on function public.email_outbox_claim(int) from public, anon, authenticated;
revoke all on function public.email_outbox_mark(uuid,text,text,uuid) from public, anon, authenticated;
