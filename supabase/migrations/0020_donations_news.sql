-- Feature: Spenden-System + News-Feed
--
-- 5 % jedes Netto-Umsatzes wandert in einen Spendentopf für einen vom
-- Kundenkollektiv gewählten gemeinnützigen Zweck. Kunden sehen ihren
-- eigenen kumulierten Anteil (Header + Kaufhistorie) und können
-- Vorschläge für Spendenempfänger machen bzw. abstimmen.
--
-- Netto-Basis: Für Snacks/Süßwaren gilt 7 %, für Getränke 19 %. Da die
-- Kaufhistorie nur den Bruttobetrag (`total_gross`) hält, rechnen wir für
-- die Demo mit dem gewichteten Durchschnitt 7 % (Grundnahrungsmittel-
-- Regelfall). In Produktion würde man `total_net` je Bon persistieren.
...
-- Seed: 3 aktive Zwecke, damit die Abstimmung nicht leer ist
insert into public.donation_causes (title, description, status) values
  ('Tafel Magdeburg',
   'Unterstützt Menschen in Magdeburg und Umgebung mit Lebensmitteln.',
   'active'),
  ('Kinderhospiz Magdeburger Elbland',
   'Hospiz-Betreuung für Kinder und Familien in Sachsen-Anhalt.',
   'active'),
  ('Feuerwehr Sülzetal',
   'Ehrenamtliche Feuerwehr in unserer Gemeinde — Ausrüstung und Jugendarbeit.',
   'active')
on conflict do nothing;

-- ── Konstante: Spendensatz ─────────────────────────────────────────────

create or replace function public.donation_rate() returns numeric
language sql immutable as $$ select 0.05::numeric $$;

-- Netto-Umsatz je Kauf (Vereinfachung: 7 % USt-Regelsatz für Lebensmittel)
...
-- ── RPCs: Spenden-Übersicht ────────────────────────────────────────────

-- Kumulierte Spende des angemeldeten Kunden.
...
-- Käufe des Kunden mit Netto-Anteil, Spendenbetrag und relativer Quote.
...
-- Liste aller Spendenzwecke inkl. Stimmenanzahl + eigenem Vote-Flag.
...
-- Kunde schlägt einen neuen Zweck vor (status = suggested).
...
-- Toggle-Vote: existiert Stimme → löschen; sonst anlegen.
...
-- Liste veröffentlichter News (jüngste zuerst).
