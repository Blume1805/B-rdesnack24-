-- ============================================================================
-- 0155 — Werbenetzwerk, Teil 2: Funktionen
-- ----------------------------------------------------------------------------
-- Fortsetzung von 0154. Rollenprüfungen folgen dem Muster aus
-- check_rollenpruefung.py: NULL wird ausdrücklich abgefangen, nie über
-- `not in`.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- Ohne jede Rollenprüfung und für `anon` freigegeben: Der Aufruf kommt von
-- Menschen, die einen QR-Code oder Coupon-Link antippen, ohne angemeldet zu
-- sein. Die Funktion kann NUR hochzählen — kein Lesezugriff, kein anderer
-- Nebeneffekt. Das größte Risiko ist ein aufgeblähter Zähler durch
-- wiederholtes Antippen oder einen Bot; das ist ein geringes Risiko und im
-- Bericht als Grenze benannt („ein Aufruf ist kein Mensch").
...
-- ----------------------------------------------------------------------------
-- Anlass-Schlüssel dieser Migration → Wert in personal_offers.source
-- ----------------------------------------------------------------------------
-- `personal_offers.source` ist eine gewachsene TEXT-Spalte mit englischen
-- Werten (`birthday`, `loyalty`, `anniversary`, `auto` — siehe
-- `PersonalOfferSource` in apps/mobile/lib/features/customer/domain/
-- entities/offer.dart). `app.coupon_anlass` ist bewusst deutsch benannt wie
-- der Rest dieses Werbenetzwerks. Beide Wortlisten sind an keiner Stelle
-- gleich, deshalb bildet diese Funktion explizit ab, statt sie textgleich zu
-- vergleichen — ein stiller `::text = ::text`-Vergleich hätte hier für vier
-- von fünf Anlässen dauerhaft 0 Treffer ergeben, ohne dass ein Fehler
-- aufgetreten wäre.
...
-- ----------------------------------------------------------------------------
-- Der Abschlussbericht — drei Zahlen, Mindestzahl 30
-- ----------------------------------------------------------------------------
-- „Ausgabe" bzw. „Aktivierung" und „Einlösung" kommen aus den bestehenden
-- Tabellen `personal_offers` und `offer_activations` — OHNE dass diese je
-- eine Kampagnen-Spalte bekommen hätten. Der Ausschluss in
-- advertising_coupon_sponsorships garantiert: Während des gebuchten
-- Zeitraums trug der Anlass genau dieses eine Logo. Damit lässt sich die
-- Zählung nachträglich über den Zeitraum herleiten, ohne dass irgendeine
-- Kaufzeile je erfährt, für welchen Sponsor sie zählt.
