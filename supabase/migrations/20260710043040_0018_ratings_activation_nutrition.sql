...
create policy ratings_read on public.product_ratings for select to authenticated using (true);
...
create index if not exists idx_offer_activ_customer on public.offer_activations(customer_id, activated_at desc);
