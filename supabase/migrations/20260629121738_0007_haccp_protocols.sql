...
create table if not exists public.temperature_logs (
  ...
  corrective_action text, photo_path text, recorded_by uuid,
...
create table if not exists public.cleaning_logs (
  ...
  agent text, notes text, photo_path text, signature_path text,
...
create table if not exists public.disposal_logs (
  ...
  photo_path text, recorded_by uuid,
  created_at timestamptz not null default now()
);
...
create table if not exists public.maintenance_logs (
  ...
  issue text not null, action text, performed_by text,
...
create table if not exists public.signatures (
  ...
  signer_id uuid, signer_name text,
...
create policy sig_read on public.signatures for select to authenticated
  using (public.is_admin() or public.auth_has_permission('haccp.cleaning')
...
create policy sig_insert on public.signatures for insert to authenticated
  with check (public.is_admin() or public.auth_has_permission('haccp.cleaning')
