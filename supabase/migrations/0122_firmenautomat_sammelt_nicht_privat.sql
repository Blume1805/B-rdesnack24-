-- ============================================================================
-- 0122 · Am Firmenautomaten sammelt niemand privat
-- ----------------------------------------------------------------------------
-- Entscheidung vom 22.08.2026, wörtlich: „Der Mitarbeiter sammelt für sich.
-- Er soll über seine Kundenkarte kaufen, und nur wenn er über den Automaten
-- kauft, der bei der Firma steht, sammelt er keine Punkte für sich, sondern
-- die Daten werden der Firma übermittelt."
--
-- Entscheidend ist damit der ORT des Automaten, nicht die Person. Derselbe
-- Mensch mit derselben Kundenkarte sammelt am öffentlichen Automaten für
-- sich und am Automaten in seiner Firma für die Firma. Das ist die Regel,
-- die hier gebaut wird.
--
-- Sie passt ohne Umweg auf das, was seit Schritt 1 und 3 schon steht:
-- `locations.kind = 'business'` sagt, dass ein Ort Firmengelände ist, und
-- `business_locations` sagt, welcher Firma er gehört. Beides musste nicht
-- erfunden werden.
--
-- ----------------------------------------------------------------------------
-- Warum BEIDE Bedingungen geprüft werden
--
-- Die Zuordnung greift nur, wenn der Ort `kind = 'business'` trägt UND der
-- Firma über `business_locations` zugeordnet ist UND der Käufer dort aktives
-- Mitglied ist.
--
-- Die erste Bedingung ist kein Zierrat. `business_locations` kann auch einen
-- öffentlichen Ort für eine Firma freigeben — etwa damit deren Mitarbeiter
-- dort einkaufen dürfen. Ohne die Prüfung auf `kind = 'business'` würde ein
-- privater Feierabendkauf am öffentlichen Automaten still zum Firmenkauf,
-- nur weil der Arbeitgeber diesen Ort freigeschaltet hat. Der Auftrag sagt
-- ausdrücklich „der Automat, der bei der Firma steht".
--
-- ----------------------------------------------------------------------------
-- Ein Trigger statt zwei
--
-- Migration 0121 legte `app.purchase_business_pruefen()` als Prüfung an.
-- Die Herleitung kommt jetzt in DIESELBE Funktion, statt als zweiter
-- Trigger daneben. Grund: Trigger derselben Art feuern in alphabetischer
-- Reihenfolge ihres Namens. Eine Herleitung, die nach der Prüfung liefe,
-- prüfte nichts mehr — und ob sie davor liegt, hinge dann an einem
-- Buchstaben im Namen. Eine Funktion, zwei Schritte in fester Reihenfolge,
-- kein Verlass auf Alphabet.
-- ============================================================================
...
begin
  -- ── Schritt 1: Herleiten ────────────────────────────────────────────────
  -- Nur wenn niemand die Firma ausdrücklich gesetzt hat. Ein gesetzter Wert
  -- wird nicht überschrieben, sondern in Schritt 2 geprüft.
  if new.business_id is null and new.machine_id is not null then
    ...
    -- Dann ist nicht entscheidbar, wer die Rechnung trägt. Lieber
    -- abbrechen als würfeln — eine falsche Rechnung fällt niemandem auf.
    if v_treffer > 1 then
      raise exception
...
comment on function app.purchase_business_pruefen() is
  'Leitet die Firma aus dem Standort des Automaten her (Firmengelände + '
  'Freigabe + aktive Mitgliedschaft) und prüft anschliessend jede '
  'Firmenzuordnung. Greift auch bei service_role, wo keine Zugriffsregel '
  'greift.';

-- ============================================================================
-- Wer für sich sammelt, sammelt nur privat
-- ============================================================================
-- Ab hier bekommt jede Funktion, die Käufe für den KUNDEN auswertet, den
-- Zusatz `business_id is null`. Neun Stellen waren es; gesucht wurden sie
-- nicht aus dem Gedächtnis, sondern über eine Abfrage auf `pg_proc` nach
-- allen Funktionen, die `purchases` je Kunde lesen.
--
-- Nicht angefasst werden die Auswertungen des Betriebs — `finance_kpis`,
-- `finance_summary`, `datev_export_rows`, `create_invoice_for_purchase`,
-- `export_my_data`. Für Umsatz, Buchhaltung und Auskunft zählt jeder Kauf,
-- gleich wer ihn bezahlt.
--
-- `my_receipts` bleibt ebenfalls unverändert: Der Mitarbeiter hat den Kauf
-- getätigt und muss sehen können, was er gezogen hat. Ein Beleg ist keine
-- Prämie.
...
-- ----------------------------------------------------------------------------
-- Meilenstein-Gutscheine
-- ----------------------------------------------------------------------------
-- Auch die Produktauswahl für den Gutschein wird eingeschränkt: Ein Gutschein,
-- der auf dem Firmeneinkauf beruht, wäre wieder ein privater Vorteil aus
-- fremder Rechnung.
...
-- ----------------------------------------------------------------------------
-- Status, Abzeichen, Aufgabenanzeige
-- ----------------------------------------------------------------------------
-- Betrifft auch den Statusumsatz und die Abzeichen: Wer sich über
-- Firmeneinkäufe zu Gold hocharbeiten könnte, bekäme einen lebenslangen
-- privaten Rabatt aus fremder Rechnung.
...
-- ----------------------------------------------------------------------------
-- Der Auftrag sagt, die Daten des Firmenkaufs gehen an die Firma. Dann dürfen
-- sie auch nicht das private Angebot prägen: Sonst verriete der Vorschlag im
-- Kundenbereich, was am Arbeitsplatz gekauft wurde.
--
-- Für das KI-Register ändert sich nichts: Die Datenkategorie bleibt „Käufe
-- der letzten 90 Tage", ihr Umfang wird nur enger.
...
-- Spenden
-- ----------------------------------------------------------------------------
-- „Deine Käufe haben X € gespendet" ist eine persönliche Zuschreibung. Sie
-- darf nicht auf Geld beruhen, das die Firma bezahlt hat.
--
-- Der GESAMTTOPF bleibt vollständig: Gespendet wurde das Geld ja trotzdem.
-- Deshalb wird in `donation_pool_summary` ausschliesslich der eigene Anteil
-- eingeschränkt, nicht `app_total`.
...
as $function$
  ...
  app_total as (
    -- Bewusst OHNE Einschränkung: Der Gesamttopf enthält jeden Kauf.
    select coalesce(sum(public.purchase_donation(total_gross)),0) as v
    from public.purchases
  ),
  automat as (
