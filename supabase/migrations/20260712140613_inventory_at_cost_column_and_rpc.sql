...
begin
  ...
  base as (
    select
      ...
      greatest(0, i.quantity
        - coalesce(ir.refill,0) + coalesce(ir.sales,0) + coalesce(ir.disposal,0)
        - coalesce(ir.correction,0) - coalesce(da.v,0))::int as start_qty,
  ...
  select
    ...
    case when b.expiry_date is null then null
         else (b.expiry_date - (p_to at time zone 'UTC')::date) end as mhd_days_left,
    app.mhd_writedown_pct(
      case when b.expiry_date is null then null
           else (b.expiry_date - (p_to at time zone 'UTC')::date) end
    ) as mhd_pct,
    round(b.end_qty * b.unit_cost
      * app.mhd_writedown_pct(
          case when b.expiry_date is null then null
               else (b.expiry_date - (p_to at time zone 'UTC')::date) end
        ) / 100.0, 2) as mhd_discount,
    round(b.end_qty * b.unit_cost
      * (1 - app.mhd_writedown_pct(
              case when b.expiry_date is null then null
                   else (b.expiry_date - (p_to at time zone 'UTC')::date) end
            ) / 100.0), 2) as net_value
  from base b
