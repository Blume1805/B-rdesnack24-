...
create table if not exists public.customers (
  ...
  billing_street text, billing_zip text, billing_city text,
...
create table if not exists public.recommendations (
  ...
  customer_id uuid not null references public.customers(id) on delete restrict,
  product_id uuid not null references public.products(id) on delete restrict,
  score numeric(5,2) not null default 0, reason text,
  created_at timestamptz not null default now()
);
...
create table if not exists public.consents (
  ...
  type app.consent_type not null, granted boolean not null,
...
create policy pitems_read on public.purchase_items for select to authenticated
  using (public.is_admin() or public.auth_has_permission('customers.manage')
    or exists (select 1 from public.purchases p where p.id = purchase_id and p.customer_id = auth.uid()));
