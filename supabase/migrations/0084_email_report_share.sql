-- ============================================================================
-- 0084 · Ablaufende Share-Links für redigierte E-Mail-Testreports
-- ----------------------------------------------------------------------------
-- HERKUNFT
-- Aus `supabase/sql/email_report_share.sql` der Loveable-App, dort nie
-- ausgeführt. Übernahme als Migration auf Entscheidung des Betriebs; zur
-- Begründung siehe Kopf von 0083.
--
-- Der Entwurf ist gut: RLS auf beiden Tabellen, öffentlicher Abruf
-- ausschließlich über eine SECURITY-DEFINER-Funktion mit Ablauf, Widerruf
-- und Aufruflimit, und **jeder** Zugriff wird protokolliert — auch
-- `not_found`, wodurch das Raten von Tokens sichtbar wird. Der Entzug geht
-- gegen `public`, nicht nur gegen `anon`; genau der Fallstrick, über den
-- 0075 gestolpert ist.
--
-- EINE ÄNDERUNG GEGENÜBER DER VORLAGE
-- `fetch_email_report_share` ist für `anon` freigegeben — das muss sie sein,
-- sonst funktionieren die Links nicht. Sie schreibt dabei aber bei **jedem**
-- Aufruf eine Protokollzeile, auch bei unbekanntem Token, und übernimmt
-- `user_agent`, `referer` und `client_hint` ungeprüft und unbegrenzt aus der
-- Anfrage.
--
-- Damit kann jeder Fremde ohne Anmeldung beliebig viele Zeilen mit beliebig
-- langen Zeichenketten in die Datenbank schreiben. Das Protokollieren
-- fehlgeschlagener Versuche ist richtig und bleibt — die Längen werden aber
-- gekappt (300/300/120 Zeichen). Das nimmt dem Missbrauch die Wirkung, ohne
-- den Erkennungswert zu verlieren.
--
-- Ein Mengenlimit fehlt weiterhin. Sinnvoll wäre ein Aufräumjob (die dritte
-- Datei der Vorlage, `email_report_share_cleanup.sql`, kommt als eigene
-- Migration nach) plus ggf. Rate-Limiting an der API-Schicht.
-- ============================================================================

create table if not exists public.email_report_share (
  id uuid primary key default gen_random_uuid(),
  -- Zufälliges Token, das im Link steht (nur wer den Link hat, kommt rein).
  token text not null unique,
  -- Ausschliesslich die redigierte Fassung des Reports.
...
create table if not exists public.email_report_share_access (
  ...
  -- ok | expired | revoked | not_found | view_limit
...
-- Öffentlicher Abruf: prüft Ablauf/Widerruf/View-Limit, protokolliert JEDEN
-- Zugriff und gibt nur bei Erfolg die redigierte Fassung heraus.
...
declare
  ...
  v_views integer := 0;
  v_outcome text;
  -- Gekappt, weil die Werte ungeprüft aus einer anonymen Anfrage stammen.
