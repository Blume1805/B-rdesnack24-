...
--
-- Liefert je Kalendertag und Steuersatz die Brutto-Summe der App-Verkäufe
-- (purchases × purchase_items × products.tax_rate). Der Client baut daraus
-- die DATEV-Buchungszeilen: Kasse (SKR 03 Konto 1000) an Erlöse 7 %
-- (8300) bzw. 19 % (8400) — konsistent zur Verfahrensdokumentation Kap. 5.
--
-- Positionssummen statt purchases.total_gross, damit die USt-Aufteilung
-- exakt aus den Artikeln kommt (gemischte Warenkörbe!). Guard wie alle
-- Finance-RPCs: nur system_admin/shareholder; Least Privilege gemäß
-- Audit 2026-07 (kein PUBLIC/anon-EXECUTE).
