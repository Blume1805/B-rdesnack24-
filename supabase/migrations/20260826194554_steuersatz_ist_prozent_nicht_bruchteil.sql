...
--
-- Abgefragt, nicht angenommen: 38 Produkte tragen 7.0, 24 tragen 19.0.
-- Kein einziges traegt 0.07 oder 0.19.
--
-- ZWEI STELLEN RECHNETEN FALSCH
--
-- 1. app.business_invoice_basis (0148, von mir) nahm den Wert als Bruchteil:
--    netto * (1 + 7.0) haette das Achtfache ergeben, und der ausgewiesene
--    Satz waere "700 %" gewesen. Beim ersten Test mit echten Produkten
--    aufgefallen.
--
-- 2. public.create_invoice_for_purchase (bestehend) vergleicht
--    `p.tax_rate = 0.07` und `= 0.19`. Beide Vergleiche treffen nie. Damit
--    fiel jede Rechnung in den Rueckfall "keine Positionen -> alles 7 %"
--    (total_gross / 1.07) — auch fuer die 24 Produkte mit 19 %. Eine
--    Rechnung ueber ein Getraenk haette 7 % statt 19 % ausgewiesen.
--
--    `invoices` hat 0 Zeilen; es ist also keine falsche Rechnung
--    hinausgegangen. Der Fehler war bisher folgenlos und waere es beim
--    ersten Unternehmer-Kunden nicht mehr gewesen.
--
-- Beide Stellen rechnen jetzt mit Prozent.
