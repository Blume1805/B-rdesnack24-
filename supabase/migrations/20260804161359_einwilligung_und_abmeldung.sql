...
comment on table public.email_topic is
  'Themen der Werbepost. Vertragspost taucht hier NICHT auf — sie ist nicht abbestellbar und braucht keine Einwilligung.';

insert into public.email_topic (key, label, description, sort) values
  ('produkt_neuigkeiten', 'Neue Funktionen und App-Updates',
   'Wenn es in der App etwas Neues gibt — höchstens ein paar Mal im Jahr.', 10),
  ('aktionen', 'Aktionen und Angebote',
   'Saisonale Aktionen, Rabatte und Aktionen an den Automaten.', 20),
  ('geburtstag', 'Geburtstagsgruß',
   'Einmal im Jahr ein Gutschein zum Geburtstag.', 30)
...
comment on table public.email_consent_event is
  'Nur anhängen, nie ändern. Trägt die Nachweispflicht aus Art. 7 Abs. 1 DSGVO. Der aktuelle Stand wird daraus abgeleitet, nicht gespeichert.';
...
begin
  raise exception
    'email_consent_event ist ein Nachweisprotokoll und wird nur angehängt (Art. 7 Abs. 1 DSGVO). Ein Widerruf ist eine NEUE Zeile mit granted = false.';
end;
$function$;
...
begin
  ...
  if p_proof_text is null or length(trim(p_proof_text)) < 10 then
    raise exception 'Der Einwilligungstext muss mitgegeben werden — er ist der Nachweis.';
  end if;
...
comment on table public.email_unsubscribe_token is
  'Ein dauerhaftes Merkmal je Person für den Abmeldelink in Werbemails. Keine Policy, keine Rechte: ausschliesslich über SECURITY-DEFINER-Funktionen erreichbar.';
