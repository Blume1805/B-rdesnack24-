-- ============================================================================
-- 0118 · Die versprochenen Bonuspunkte werden eingelöst
-- ----------------------------------------------------------------------------
-- Befund vom 22.08.2026. Der Belohnungs-Bildschirm zeigt zu zwei Aufgaben
-- fett und mit Geschenk-Symbol:
--
--     Belohnung: +300 Bonuspunkte        (Wochen-Snacker, 3 Käufe/Woche)
--     Belohnung: +200 Bonuspunkte        (Morgenmuffel-Kur, 3 Käufe 7–9 Uhr)
--
-- Nachgesehen statt vermutet: `challenges.reward_text` wird ausschliesslich
-- angezeigt. `my_gamification_status()` liest die Spalte und reicht sie an
-- die Oberfläche durch; keine einzige Zeile Code schreibt diese Punkte je
-- gut. Die Suche nach `challenge` über alle Migrationen findet neben 0058
-- (Anlage) nur 0060 (Anzeige) und 0096 (ein Index).
--
-- Die beiden anderen Aufgaben halten dagegen, was sie versprechen: Sie
-- versprechen Abzeichen, und `explorer` beziehungsweise `taster` werden in
-- derselben Funktion tatsächlich als `earned` gemeldet, mit denselben
-- Schwellen (3 Automaten, 5 Produkte). Der Fehler betrifft also genau die
-- zwei Aufgaben, die Punkte zusagen.
--
-- ----------------------------------------------------------------------------
-- Warum die Beträge unverändert bleiben
--
-- 300 und 200 sind nicht neu erfunden. Sie stehen seit Migration 0058 in der
-- Datenbank und werden dem Kunden seitdem angezeigt. Sie einzulösen behebt
-- einen Mangel; sie zu ändern wäre eine Produktentscheidung. Deshalb ändert
-- diese Migration keinen Betrag.
--
-- Vergeben wird EINMALIG je Kunde und Aufgabe — Entscheidung vom 22.08.2026.
-- Der Zusagetext nennt keine Wiederholung, und die Einmalvergabe ist der
-- kleinste Eingriff in die Punkte-Wirtschaft, die bei den Login-Punkten
-- festgelegt wurde (10/Tag, 100 für die volle Woche, Reset zum Monatsersten).
-- Zum Vergleich, damit die Grössenordnung sichtbar ist: Tägliches Einloggen
-- bringt 160 Punkte je Woche. Eine wiederholte Wochenprämie von 300 wäre
-- fast das Doppelte davon und hätte das Gewicht deutlich verschoben.
--
-- Die Einmaligkeit ist nicht als Bedingung formuliert, sondern als
-- Primärschlüssel: `(customer_id, challenge_code)`. Eine doppelte Gutschrift
-- ist damit nicht unwahrscheinlich, sondern unmöglich.
--
-- ----------------------------------------------------------------------------
-- Nebenbei behoben: die doppelte Messlogik
--
-- Die Fortschrittswerte der Aufgaben standen bisher als vier fest
-- verdrahtete Abfragen in `my_gamification_status()`. Eine Vergabe daneben
-- zu stellen, hätte sie ein zweites Mal geschrieben — und zwei Kopien
-- derselben Regel laufen früher oder später auseinander. Sie stehen jetzt
-- einmal in `app.challenge_progress()`, und beide Seiten lesen von dort.
--
-- Dabei fiel auf, dass `challenges.window_days` gar nicht benutzt wurde:
-- Die Zeitfenster waren als `interval '7 days'` beziehungsweise
-- `'30 days'` fest in der Funktion verdrahtet. Wer die Spalte änderte,
-- änderte nichts. Jetzt steuert sie die Messung wirklich.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- Was eine Aufgabe an Punkten bringt
-- ----------------------------------------------------------------------------
-- Eigene Spalte statt einer Auswertung von `reward_text`: Der Anzeigetext ist
-- für Menschen da. Eine Zahl aus ihm herauszulesen hiesse, jede spätere
-- Umformulierung zu einer stillen Betragsänderung zu machen.
...
-- ----------------------------------------------------------------------------
-- Die vergebenen Prämien
-- ----------------------------------------------------------------------------
-- Kein `deleted_at`, kein `updated_at`: Eine Gutschrift wird nicht geändert
-- und nicht zurückgenommen. Der Primärschlüssel trägt die Einmaligkeit.
...
-- Wie `customer_login_days` und `customer_card`: Zeilensicherheit an, KEINE
-- Regel und KEINE Tabellenberechtigung. Der Kunde kommt an diese Daten
-- ausschliesslich über die geprüften Funktionen unten. Was keine Regel hat,
-- kann keine falsche Regel haben.
...
-- ----------------------------------------------------------------------------
-- Der Fortschritt — eine Quelle für Anzeige und Vergabe
-- ----------------------------------------------------------------------------
  ...
  cross join lateral (
    select case c.metric
      ...
      when 'morning_count' then (
        -- 7 bis 8 einschliesslich heisst 07:00 bis 08:59 — also „zwischen
        -- 7 und 9 Uhr", wie die Aufgabe es zusagt.
        select count(*) from public.purchases p
         where p.customer_id = p_customer
           and p.purchased_at >= now() - make_interval(days => c.window_days)
...
-- Die Vergabe
-- ----------------------------------------------------------------------------
-- Kein Abo-Vorbehalt, mit Absicht: Für die Punkte-Wirtschaft gilt „sammeln
-- darf jeder, den Gutschein bekommt nur, wer zahlt". Das Abo wird eine Ebene
-- höher geprüft, in `app.grant_loyalty_bonuses()`.
...
begin
  ...
  -- Punktesystem nicht teil. Dieselbe Prüfung wie in record_daily_login();
  -- ohne sie liefe der Insert in eine Fremdschlüsselverletzung.
...
-- ----------------------------------------------------------------------------
-- Was davon in den laufenden Monat zählt
-- ----------------------------------------------------------------------------
-- Die Punkte werden zum Monatsersten zurückgesetzt, wie alle anderen auch.
-- Eine einmalige Prämie zählt also in dem Monat, in dem sie vergeben wurde.
--
-- Gerechnet wird in Berliner Zeit, nicht in UTC: Die Datenbank läuft auf UTC,
-- und `app.heute()` schneidet den Monat auf Berliner Tagesgrenzen. Ohne die
-- Umrechnung fiele eine Gutschrift vom 1. um 00:30 Uhr deutscher Zeit in den
-- Vormonat, den Monatsanfang bestimmt aber `app.heute()`.
...
-- ============================================================================
-- Die vorhandenen Funktionen ziehen nach
-- ============================================================================

-- ----------------------------------------------------------------------------
-- Anzeige: liest jetzt aus derselben Quelle wie die Vergabe
-- ----------------------------------------------------------------------------
-- Geändert wird ausschliesslich der Aufgaben-Block. Stufen, Rabattanzeige und
-- Abzeichen bleiben Zeile für Zeile, wie sie waren — sie sind nicht Gegenstand
-- dieses Befundes, und eine Migration, die nebenbei aufräumt, macht ihren
-- eigenen Fehler unauffindbar.
...
begin
  ...
  -- Aufgaben: Fortschritt aus app.challenge_progress(), dazu zwei neue
  -- Felder. `reward_points` sagt der Oberfläche, ob es überhaupt Punkte
  -- gibt; `awarded` sagt, ob sie schon geflossen sind — damit „+300
  -- Bonuspunkte" nicht weiter als offenes Versprechen dasteht, nachdem es
  -- eingelöst wurde.
  select jsonb_agg(jsonb_build_object(
...
-- ----------------------------------------------------------------------------
-- Punktestand: Aufgaben-Prämien zählen mit
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Meilensteinvergabe: dieselbe Summe wie in der Anzeige
-- ----------------------------------------------------------------------------
-- Nur die Punkteberechnung ändert sich. Ginge sie hier auseinander, zeigte
-- die App einen erreichten Meilenstein, ohne dass ein Gutschein entsteht.
...
-- ----------------------------------------------------------------------------
-- Der Einstiegspunkt: bei jedem Öffnen der App
-- ----------------------------------------------------------------------------
-- Die Vergabe hängt an `record_daily_login()`, weil die App diese Funktion
-- ohnehin bei jedem Start ruft. Sie läuft dabei UNABHÄNGIG davon, ob der
-- heutige Tag neu ist: Eine Aufgabe wird durch einen KAUF erfüllt, nicht
-- durch einen Login. Liefe die Vergabe nur am ersten Login des Tages, bliebe
-- eine am Nachmittag erfüllte Aufgabe bis zum nächsten Morgen unbelohnt.
--
-- Ebenso musste die Meilensteinvergabe erweitert werden: Sie lief bisher nur,
-- wenn ein neuer Login-Tag dazukam. Da Aufgaben-Prämien den Punktestand
-- ebenfalls über einen Meilenstein heben können, läuft sie jetzt auch dann,
-- wenn Prämien geflossen sind.
...
begin
  ...
  return v_woche || jsonb_build_object(
    ...
    -- Neu vergebene Aufgaben-Prämien. 0 heisst: nichts Neues, keine
    -- Animation. Bewusst getrennt von `awarded`, damit die Oberfläche
    -- „10 Punkte fürs Einloggen" und „300 Punkte für den Wochen-Snacker"
    -- unterscheiden und einzeln benennen kann.
...
insert into public.ki_register (
  ...
  -- `datenkategorien` ist ein Textfeld-Array mit kurzen Schlüsseln, keine
  -- Prosa — so wie `{login_tag}` beim Eintrag zu den Login-Punkten.
  array['kaufzeitpunkt', 'kaufanzahl', 'automat', 'produkt'],
  true,   -- personenbezug
  true,   -- profiling: bewertet Kaufverhalten, um eine Prämie auszulösen
  false,  -- ist_ki_system
  -- Die erlaubten Werte stehen als Prüfregeln auf der Tabelle. Nachgesehen
  -- statt geraten: ai_act_kategorie aus {kein_ki_system, minimal,
  -- transparenz, hochrisiko, verboten}, dsgvo_risiko aus {niedrig, mittel,
  -- hoch, unzulaessig}.
  'kein_ki_system',
  'niedrig',
  'Art. 6 Abs. 1 lit. b DSGVO (Vertragserfüllung)',
  -- Kennzeichnung nötig UND umgesetzt: Der Kunde sieht eine Gutschrift von
  -- 300 Punkten, ohne dass ihm sonst irgendwo gesagt würde, wodurch sie
  -- ausgelöst wurde. Die Ergänzung im AiInfoScreen liegt im selben Commit,
  -- wie es CLAUDE.md verlangt. Die Prüfregel `ki_kennzeichnung_vor_gruen`
  -- lässt „gruen" nur zu, wenn beides zusammenpasst.
  true,   -- kennzeichnung_noetig
  true,   -- kennzeichnung_umgesetzt
  false,  -- freigabe_noetig
  false,  -- minderjaehrige
  false,  -- manipulationsrisiko
  false,  -- deepfake_risiko
  'gruen',
  true,
