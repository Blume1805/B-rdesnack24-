...
class _B2bCustomersScreenState extends ConsumerState<B2bCustomersScreen> {
  ...
  Future<void> _load() async {
    setState(() => _busy = true);
    try {
      // Direkt gegen die RPC, damit die Gesellschafter-App nicht an der
      // Kunden-App haengt. Die Berechtigung prueft `business_customers_csv`
      // selbst -- ein Mitarbeiterkonto bekommt hier nichts zu sehen.
      final csv = await ref
              .read(supabaseClientProvider)
              .rpc('business_customers_csv') as String? ??
          '';
